...
function cleanClosest(SpawnPointParams, mapIndex, player) {
    const map = constants_1.configLocations[mapIndex];
    const mapCullingNearPointValue = player
        ? mapConfig_json_1.default[map].mapCullingNearPointValuePlayer
        : mapConfig_json_1.default[map].mapCullingNearPointValue;
    ...
    if (config_json_1.default.debug) {
        ...
        console.log(map, filteredParams.length, ">", actualCulled.length, "Reduced to " +
            ...
            "% of original spawns", player ? "player" : "bot"); // high, low}
    }
...
const removeClosestSpawnsFromCustomBots = (CustomBots, SpawnPointParams, map, mapConfigMap) => {
    ...
    const mapCullingNearPointValue = mapConfig_json_1.default[mapConfigMap].mapCullingNearPointValue;
