...
let ModdedImportHelper = class ModdedImportHelper {
    ...
    attachmentBlacklist;
    clothingBlacklist;
    modScope000Whitelist = [];
    itemShouldNotGetModdedChildrenAttachments = [];
    invalidModAttachments = [];
    invalidModEquipment = [];
    tier4Optics;
    foldingModSights;
    allImportedAttachments;
    numberOfAttachments;
    ...
    tiersTable = [];
    initialize() {
        if (ModConfig_1.ModConfig.config.compatibilityConfig.initalTierAppearance < 1 || ModConfig_1.ModConfig.config.compatibilityConfig.initalTierAppearance > 7) {
            this.apbsLogger.log(Logging_1.Logging.WARN, `Config value for "initialTierAppearance" is invalid. Must be 1-7. Currently configured for ${ModConfig_1.ModConfig.config.compatibilityConfig.initalTierAppearance}`);
            return;
        }
        if (ModConfig_1.ModConfig.config.compatibilityConfig.enableModdedWeapons)
            this.buildVanillaWeaponList();
        if (ModConfig_1.ModConfig.config.compatibilityConfig.enableModdedEquipment)
            this.buildVanillaEquipmentList();
        if (ModConfig_1.ModConfig.config.compatibilityConfig.enableModdedClothing && !ModConfig_1.ModConfig.config.pmcBots.additionalOptions.seasonalPmcAppearance)
            this.buildVanillaClothingList();
        if (ModConfig_1.ModConfig.config.compatibilityConfig.enableModdedAttachments) {
            this.buildModAttachments();
            if (this.invalidModAttachments.length > 0)
                this.apbsLogger.log(Logging_1.Logging.DEBUG, `${this.invalidModAttachments.length} Invalid Attachment ItemIDs found in mods: ${JSON.stringify(this.invalidModAttachments)}`);
            if (this.invalidModEquipment.length > 0)
                this.apbsLogger.log(Logging_1.Logging.DEBUG, `${this.invalidModEquipment.length} Invalid Weapon/Equipment ItemIDs found in mods: ${JSON.stringify(this.invalidModEquipment)}`);
            if (this.allImportedAttachments.length > 0)
                this.apbsLogger.log(Logging_1.Logging.WARN, `     Importing ${this.allImportedAttachments.length} Modded Attachments to ${this.numberOfAttachments} mount points on vanilla weapons...`);
        }
    }
    ...
    buildVanillaEquipmentList() {
        ...
        const armbands = {};
        VanillaItemLists_1.vanillaItemList.equipment.ArmBand.forEach(element => armbands[element] = this.getItem(element));
        ...
        this.getModdedItems(armbands, BaseClasses_1.BaseClasses.ARMBAND, "ArmBands");
    }
        ...
        for (const object in this.tierInformation.tiers) {
            ...
            for (const item in clothingList) {
                if (clothingList[item]._props.Side.includes("Bear")) {
                    ...
                    this.apbsLogger.log(Logging_1.Logging.DEBUG, `[Clothing Tier ${tierNumber}] Added ${clothingList[item]._id} to ${clothingList[item]._props.BodyPart} BEAR.`);
                }
                if (clothingList[item]._props.Side.includes("Usec")) {
                    ...
                    this.apbsLogger.log(Logging_1.Logging.DEBUG, `[Clothing Tier ${tierNumber}] Added ${clothingList[item]._id} to ${clothingList[item]._props.BodyPart} USEC.`);
                }
            }
        }
    }
    ...
    getSetModdedWeaponDetails(weaponPool) {
        ...
        for (const weapon in weaponPool) {
            ...
            if (!this.doesItemExist(weaponId, true))
                continue;
    ...
    getSetModdedEquipmentDetails(equipmentPool) {
        ...
        for (const equipment in equipmentPool) {
            ...
            let equipmentSlot;
            // Check if item is actually valid and in the database, skip if it isn't
            if (!this.doesItemExist(equipmentId, true))
                continue;
            ...
            if (equipmentParent == "5448e5284bdc2dcb718b4567") {
                ...
                if ((equipmentSlots.length == 0 || equipmentSlots == undefined))
                ...
                for (const slot in defaultInventorySlots) {
                    ...
                    if (slotFilter.includes(equipmentId))
                        equipmentSlot = "ArmBand";
                    if (ModConfig_1.ModConfig.config.compatibilityConfig.PackNStrap_UnlootablePMCArmbandBelts) {
            ...
            if (this.itemHelper.isOfBaseclass(equipmentId, BaseClasses_1.BaseClasses.ARMBAND))
                equipmentSlot = "ArmBand";
    ...
    pushWeaponToTiers(weaponId, weaponType, parentClass) {
        ...
        let range = "";
        const pmcWeight = ModConfig_1.ModConfig.config.compatibilityConfig.pmcWeaponWeights <= 0 ? 1 : ModConfig_1.ModConfig.config.compatibilityConfig.pmcWeaponWeights;
        const scavWeight = ModConfig_1.ModConfig.config.compatibilityConfig.scavWeaponWeights <= 0 ? 1 : ModConfig_1.ModConfig.config.compatibilityConfig.scavWeaponWeights;
        const defaultWeight = ModConfig_1.ModConfig.config.compatibilityConfig.followerWeaponWeights <= 0 ? 1 : ModConfig_1.ModConfig.config.compatibilityConfig.followerWeaponWeights;
        ...
        for (const object in this.tierInformation.tiers) {
            ...
            if (tierNumber < ModConfig_1.ModConfig.config.compatibilityConfig.initalTierAppearance)
                continue;
        ...
        this.apbsLogger.log(Logging_1.Logging.DEBUG, `[Tier${ModConfig_1.ModConfig.config.compatibilityConfig.initalTierAppearance}+] Added ${weaponId} to ${range} weapons.`);
    }
    pushEquipmentToTiers(itemID, equipmentSlot, gridLength, equipmentSlotsLength) {
        ...
        this.apbsLogger.log(Logging_1.Logging.DEBUG, `[Tier${ModConfig_1.ModConfig.config.compatibilityConfig.initalTierAppearance}+] Added ${itemID} to ${equipmentSlot}.`);
    }
    pushItemAndPrimaryMods(itemID, itemSlots) {
        ...
        for (const slot in itemSlots) {
            ...
            if (slotFilter.includes("5649a2464bdc2d91118b45a8")) {
                if (ModConfig_1.ModConfig.config.compatibilityConfig.enableMPRSafeGuard) {
                    slotFilter = ["5649a2464bdc2d91118b45a8"];
                }
                else {
                    const baseClassArrayToFilterOut = [
                        BaseClasses_1.BaseClasses.COLLIMATOR, BaseClasses_1.BaseClasses.OPTIC_SCOPE, BaseClasses_1.BaseClasses.ASSAULT_SCOPE, BaseClasses_1.BaseClasses.COMPACT_COLLIMATOR, BaseClasses_1.BaseClasses.MOUNT, BaseClasses_1.BaseClasses.SPECIAL_SCOPE
                    ];
            ...
            for (const item in slotFilter) {
                ...
                if (this.shouldItemBeSkipped(itemID, slotFilterItem, slotName))
                    continue;
                ...
                if (!this.tierInformation.tier1mods[itemID][slotName].includes(slotFilterItem)) {
                    ...
                    */
                    ...
                    if (this.itemShouldNotGetModdedChildrenAttachments.includes(slotFilterItem))
                        continue;
    ...
    recursivePushChildrenMods(parentSlotFilterItem, standaloneAttachment = false) {
        ...
        for (const slot in parentSlotSlots) {
            ...
            for (const item in slotFilter) {
                const slotFilterItem = slotFilter[item];
                if (this.shouldItemBeSkipped(parentSlotItemID, slotFilterItem, slotName, standaloneAttachment))
                    continue;
                ...
                if (!this.tierInformation.tier1mods[parentSlotItemID][slotName].includes(slotFilterItem)) {
                    ...
                    if (ModConfig_1.ModConfig.config.compatibilityConfig.enableModdedAttachments && standaloneAttachment) {
                        this.apbsLogger.log(Logging_1.Logging.DEBUG, `Found mod attachment to import. ParentSlotItem: ${parentSlotItemID} | Slot: ${slotName} | Attachment: ${slotFilterItem}`);
                        if (!this.allImportedAttachments.includes(slotFilterItem))
                            this.allImportedAttachments.push(slotFilterItem);
                        this.numberOfAttachments++;
                    }
                    // Push any children mods
                    if (this.allowedToBeRecursive(slotFilterItem))
                        this.recursivePushChildrenMods(slotFilterItem, standaloneAttachment);
                }
            }
        }
    }
    pushCalibersToAmmoPools(itemID) {
        ...
        if (!Object.keys(this.tierInformation.tier1ammo.scavAmmo).includes(itemCaliber) && itemCaliber != undefined) {
            ...
            this.apbsLogger.log(Logging_1.Logging.WARN, `[CALIBER] New caliber found in weapon, but could not find details. Item: ${itemID} | Caliber: ${itemCaliber}`);
        }
    }
    pushAmmoToTier(caliber, itemID) {
    ...
    pushStandaloneAttachmentsToWeapons(weaponID, slotName, slotItem) {
        ...
        if (!this.tierInformation.tier4mods[weaponID][slotName].includes(slotItem)) {
            if (!highTierItem || lowTierItem)
                this.tierInformation.tier1mods[weaponID][slotName].push(slotItem);
            if (!highTierItem || lowTierItem)
                this.tierInformation.tier2mods[weaponID][slotName].push(slotItem);
            if (!highTierItem || lowTierItem)
                this.tierInformation.tier3mods[weaponID][slotName].push(slotItem);
            if (!lowTierItem || highTierItem)
                this.tierInformation.tier4mods[weaponID][slotName].push(slotItem);
            if (!lowTierItem || highTierItem)
                this.tierInformation.tier5mods[weaponID][slotName].push(slotItem);
            if (!lowTierItem || highTierItem)
                this.tierInformation.tier6mods[weaponID][slotName].push(slotItem);
            if (!lowTierItem || highTierItem)
                this.tierInformation.tier7mods[weaponID][slotName].push(slotItem);
            // Push any children mods
            this.recursivePushChildrenMods(slotItem, true);
        }
    }
    buildModAttachments() {
        this.apbsLogger.log(Logging_1.Logging.WARN, `Checking & importing Modded Weapon Attachments...Support not granted for this feature...`);
        const items = this.databaseService.getTables().templates.items;
        const itemValues = Object.values(items);
        const weapons = itemValues.filter(x => this.itemHelper.isOfBaseclass(x._id, BaseClasses_1.BaseClasses.WEAPON));
        for (const weapon in weapons) {
            ...
            if (this.shouldWeaponBeSkipped(weaponID))
                continue;
            for (const slot in weaponSlots) {
                ...
                if (this.shouldSlotBeSkipped(slotName))
                    continue;
                // If filter has items
                if (slotFilter.length > 0) {
                    ...
                    for (const item in slotFilter) {
                        ...
                        if (this.shouldItemBeSkipped(weaponID, slotFilterItem, slotName, true))
                            continue;
                        // Slot is missing from weapon, add it
                        if (this.tierInformation.tier1mods[weaponID][slotName] == undefined) {
                            this.apbsLogger.log(Logging_1.Logging.DEBUG, `New Slot Found! Weapon: ${weaponID} | Slot: ${slotName} | Item: ${slotFilterItem}`);
                        ...
                        if (!this.tierInformation.tier1mods[weaponID][slotName].includes(slotFilterItem)) {
                            this.apbsLogger.log(Logging_1.Logging.DEBUG, `Found mod attachment to import. WeaponID: ${weaponID} | Slot: ${slotName} | Attachment: ${slotFilterItem}`);
                            if (!this.allImportedAttachments.includes(slotFilterItem))
                                this.allImportedAttachments.push(slotFilterItem);
                            this.numberOfAttachments++;
    ...
    doesItemExist(itemID, isEquipmentOrWeapon = false) {
        const itemExists = this.itemHelper.getItem(itemID)[0];
        if (!itemExists && !isEquipmentOrWeapon) {
            if (!this.invalidModAttachments.includes(itemID))
                this.invalidModAttachments.push(itemID);
        }
        if (!itemExists && isEquipmentOrWeapon) {
            if (!this.invalidModEquipment.includes(itemID))
                this.invalidModEquipment.push(itemID);
        }
        return itemExists;
    }
    tier4PlusOnly(parentID, slotName, itemID) {
        // Get Item Data for checks
        const itemData = this.itemHelper.getItem(itemID)[1];
        // Mod_Magazine check
        if (slotName == "mod_magazine") {
            if (!this.hasLowerAndUpperOptionsAvailable(parentID, slotName, 30))
                return false;
            if (itemData?._props?.Cartridges[0]?._max_count >= 30)
                return true;
        }
        if (slotName.includes("mod_stock") || slotName.includes("mod_handguard") || slotName.includes("mod_reciever")) {
            if (!this.hasLowerAndUpperOptionsAvailable(parentID, slotName, 8))
                return false;
            if (itemData?._props?.Ergonomics >= 8)
                return true;
        }
        if (slotName.includes("mod_scope") && this.tier4Optics.includes(itemID))
            return true;
        return false;
    }
    tier4MinusOnly(parentID, slotName, itemID) {
        // Get Item Data for checks
        const itemData = this.itemHelper.getItem(itemID)[1];
        // Mod_Magazine check
        if (slotName == "mod_magazine") {
    ...
    shouldWeaponBeSkipped(itemID) {
        // Weapon is blacklisted, skip
        if (this.blacklist.includes(itemID))
            return true;
        if (!this.apbsAttachmentChecker.isVanillaItem(itemID))
            return true;
        return false;
    }
    shouldSlotBeSkipped(slotName) {
        if (slotName == "mod_sight_front" || slotName == "mod_sight_rear")
            return true;
        return false;
    }
    shouldItemBeSkipped(parentID, itemID, slotName, standaloneAttachment = false) {
        // If item doesn't exist in the database, skip
        if (!this.doesItemExist(itemID))
            return true;
        // Attachment blacklist contains item
        if (this.attachmentBlacklist.includes(itemID))
            return true;
        ...
        if (isVanillaParent && isVanillaItem && ModConfig_1.ModConfig.config.compatibilityConfig.enableSafeGuard)
            return true;
        ...
        if (slotName == "mod_scope_000") {
            if (!this.modScope000Whitelist.includes(itemID))
                return true;
        }
        // Last checks only if it's standalone
        if (standaloneAttachment)
            return this.standaloneAttachmentShouldBeSkipped(itemID, slotName);
        return false;
    }
    standaloneAttachmentShouldBeSkipped(itemID, slotName) {
        slotName = slotName.toLowerCase();
        if (slotName == "mod_sight_front" || slotName == "mod_sight_rear") {
            if (!this.foldingModSights.includes(itemID))
                return true;
            return false;
        }
        return false;
    }
    allowedToBeRecursive(itemID) {
        return !this.itemShouldNotGetModdedChildrenAttachments.includes(itemID);
    }
    hasLowerAndUpperOptionsAvailable(parentID, slotName, threshholdValue) {
        // Check related items for this slot, to ensure it does have one that meets high threshhold requirements
        const parentSlot = this.itemHelper.getItem(parentID)[1]._props.Slots?.find((slot) => slot._name.toLowerCase() === slotName);
        ...
        if (hasHighRequirements && hasLowRequirements)
            return true;
        return false;
    }
};
