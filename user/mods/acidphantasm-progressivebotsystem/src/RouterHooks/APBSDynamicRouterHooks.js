...
let APBSDynamicRouterHooks = class APBSDynamicRouterHooks {
    ...
    constructor(dynamicRouterModService, itemHelper, apbsLogger, weatherGenerator, raidInformation, profileHelper) {
    ...
    logBotGeneration(outputJSON) {
        ...
        try {
            switch (outputJSON.data[0].Info.Settings.Role) {
                case "pmcBEAR":
                case "pmcUSEC":
                    this.apbsLogger.log(Logging_1.Logging.PMC, "---------------------------------------------------QB Bot spawned from cache---------------------------------------------------", `| ${logMessages[0]}`, `| ${logMessages[1]}`, `| ${logMessages[2]} ${logMessages[3]}`);
                    break;
                case "cursedassault":
                case "marksman":
                case "assault":
                    this.apbsLogger.log(Logging_1.Logging.SCAV, "---------------------------------------------------QB Bot spawned from cache---------------------------------------------------", `| ${logMessages[0]}`, `| ${logMessages[1]}`, `| ${logMessages[2]} ${logMessages[3]}`);
                    break;
                case "arenaFighterEvent":
                case "exUsec":
                    this.apbsLogger.log(Logging_1.Logging.RAIDER, "---------------------------------------------------QB Bot spawned from cache---------------------------------------------------", `| ${logMessages[0]}`, `| ${logMessages[1]}`, `| ${logMessages[2]} ${logMessages[3]}`);
                    break;
                case "bossBully":
                case "bossTagilla":
                case "bossPartisan":
                ...
                case "followerKolontaySecurity":
                    this.apbsLogger.log(Logging_1.Logging.BOSS, "---------------------------------------------------QB Bot spawned from cache---------------------------------------------------", `| ${logMessages[0]}`, `| ${logMessages[1]}`, `| ${logMessages[2]} ${logMessages[3]}`);
                    break;
                case "shooterBTR":
                case "skier":
                case "peacemaker":
                ...
                case "infectedTagilla":
                    this.apbsLogger.log(Logging_1.Logging.EVENT, "---------------------------------------------------QB Bot spawned from cache---------------------------------------------------", `| ${logMessages[0]}`, `| ${logMessages[1]}`, `| ${logMessages[2]} ${logMessages[3]}`);
                    break;
                default:
                    this.apbsLogger.log(Logging_1.Logging.DEBUG, "---------------------------------------------------QB Bot spawned from cache---------------------------------------------------", `| ${logMessages[0]}`, `| ${logMessages[1]}`, `| ${logMessages[2]} ${logMessages[3]}`);
                    break;
            }
        }
        catch (err) {
            this.apbsLogger.log(Logging_1.Logging.ERR, "QB Bot Generation LogOutput failed.", `${err.stack}`);
        }
        const timeTaken = performance.now() - start;
        this.apbsLogger.log(Logging_1.Logging.DEBUG, `${timeTaken.toFixed(2)}ms for logging QB bot ${botDetails.role} | Name: ${botDetails.name}`);
    }
...
exports.APBSDynamicRouterHooks = APBSDynamicRouterHooks = __decorate([
    ...
    __param(5, (0, tsyringe_1.inject)("ProfileHelper")),
    __metadata("design:paramtypes", [typeof (_a = typeof DynamicRouterModService_1.DynamicRouterModService !== "undefined" && DynamicRouterModService_1.DynamicRouterModService) === "function" ? _a : Object, typeof (_b = typeof ItemHelper_1.ItemHelper !== "undefined" && ItemHelper_1.ItemHelper) === "function" ? _b : Object, typeof (_c = typeof APBSLogger_1.APBSLogger !== "undefined" && APBSLogger_1.APBSLogger) === "function" ? _c : Object, typeof (_d = typeof WeatherGenerator_1.WeatherGenerator !== "undefined" && WeatherGenerator_1.WeatherGenerator) === "function" ? _d : Object, typeof (_e = typeof RaidInformation_1.RaidInformation !== "undefined" && RaidInformation_1.RaidInformation) === "function" ? _e : Object, typeof (_f = typeof ProfileHelper_1.ProfileHelper !== "undefined" && ProfileHelper_1.ProfileHelper) === "function" ? _f : Object])
